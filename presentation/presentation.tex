...
\begin{frame}{Noise Generation}
  ...
  \note{
    ...
    However, we keep in mind, that due to the applied noise there will be a loss of precision in the sensor values, degrading the app functionality.
  }
\end{frame}
...
\begin{frame}<1>[label=impl]{Implementation}
  ...
  \note<3>{
    The most important function is the random value generation function.
...
\begin{frame}{Effectiveness}
  ...
  \note{
    To test this we trained a k-nearest neighbour machine leaarning model with the recorded sensor values before and after the application of our patch.
    ...
    Tnis led to a noticable decrease in accuracy of our trained model.
  }
\end{frame}
