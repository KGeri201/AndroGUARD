...
Moreover, the collection of detailed API fingerprinting data could reveal a substantial amount of sensitive information.
...
To implement and deploy our mitigation we use the Android Application Patching Pipeline, short A2P2, for easy deployment \cite{DBLP:conf/IEEEares/Draschbacher23}.
By integrating the recommended strategies from the reviewed research papers into our patch, we aim to effectively obfuscate sensor data and prevent the extraction of identifiable information \cite{DBLP:conf/ndss/DasBC16}. 
...
An adversary can perform browser fingerprinting by analyzing various browser-specific attributes, such as the browser version, installed fonts, and other identifiable characteristics.
...
By rendering content and subsequently retrieving pixel data from the canvas, the adversary is able to construct a unique identifier based on subtle differences in how the browser displays these elements.
...
With the use of JavaScript one can also access sensor data to create sensor fingerprints.
...
A study bs \citeauthor{DBLP:conf/www/Gomez-BoixLB18}, about mobile browsers, highlights the differences the distinct behaviors in the absence of plugins like Flash \cite{DBLP:conf/www/Gomez-BoixLB18}.
...
Studies from \citeauthor{DBLP:journals/popets/DasBC18} have demonstrated that these discrepancies can be reliably measured and used to fingerprint individual devices, posing a significant threat to user privacy \cite{DBLP:journals/popets/DasBC18, DBLP:conf/ndss/DasBC16, DBLP:conf/ccs/DasBC14, DBLP:conf/ccs/0001ABP18}. 
...
We recognize the critical importance of developing advanced frameworks and mechanisms that are capable of effectively obfuscating or randomizing sensor data.
Such solutions are essential in mitigating the risk of device or user identification through fingerprinting techniques, which exploit the unique characteristics of sensor data to create identifiable profiles.
By introducing controlled randomness or masking techniques, these frameworks can disrupt the collection of consistent and traceable data points that would otherwise be used to uniquely identify and track users across sessions and platforms.

Our methodology will encompass an approach, by intercepting sensor values and modifying them before passing them over.
...
The functionality of the patch is validated through a process involving its application to an app, called \citetitle{SensorBox}~\cite{SensorBox}, which is designed to gather sensor measurements.
In order to to test not only the functionality but also the effectiveness of the patch we created a simple data gathering app~\cite{sensorprint} to collect unaltered and altered data at the same time. 
...
After applying the patch and retraining the algorithm it was only able to match the devices with 69.583\% accuracy, even with our few samples of ten devices \ref{tab:knn_results}. 
...
\begin{table}[h]
  ...
  \subfloat[Results before obfuscation]{
    \begin{tabularx}{.335\textwidth}{|lll|}
...
By increasing the noise intensity, it becomes more difficult for fingerprinting algorithms to reliably distinguish unique device signatures, thereby strengthening user privacy.
...
Conversely, reducing the noise level to preserve application functionality also diminishes the effectiveness of fingerprinting protection.
...
\begin{table}[h]
  ...
  \caption{knn classifier results after patch with increased noise level\\on a subset of testdevices}
...
Increasing the level of noise in sensor data can serve as a method to enhance value obfuscation, thereby reducing the efficiency of sensor fingerprinting techniques.
...
Excessive noise can impair many mobile and IoT applications depending on real-time, accurate sensor data for activities such as navigation, environmental monitoring, or gesture recognition, reducing user experience and the app's practical utility.
