...
With the use of JavaScript one can also access sensor data, like an accelerometers calibration imperfections, to create a fingerprint.
...
An other research, PriVaricator \cite{DBLP:conf/www/NikiforakisJL15} focusses on making the fingerprints non-deterministic, by introducing randomization to break identifiability when visiting websites multiple times.
...
By modifying the Android API requests before they reach the browser application, one can manipulate the values, queried by JavaScript inside the browser sent to the system.
...
These measures are designed to make it challenging for apps and services to collect user-related and privacy sensitive data by limiting access to certain APIs and data points, but do not protect accessible characteristics.
